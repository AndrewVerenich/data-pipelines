  ...
  private static String buildQuery(String mart) {
    ...
    if ("anomaly_flags".equals(mart)) {
      return "WITH tx AS ( "
          ...
          + "timestamp, "
          + "CASE "
          + "WHEN std_amount IS NOT NULL AND amount > mean_amount + 2 * std_amount THEN 'high_amount' "
          + "WHEN hour(timestamp) BETWEEN 0 AND 5 THEN 'night_activity' "
          + "WHEN tx_in_10m > 3 THEN 'high_velocity' "
          + "ELSE NULL "
          + "END AS anomaly_reason "
          + "FROM tx "
          + "WHERE "
          + "(std_amount IS NOT NULL AND amount > mean_amount + 2 * std_amount) "
